...
def get_browser_process_id(session) -> Optional[int]:
    ...
    try:
        ...
        logger.info("   🔍 Checking session for cdp_url...")
        ...
        if not port:
            ...
            if stored_port:
                ...
                logger.info("   🔍 cdp_url is None/Empty and no stored port available")
        ...
        if port:
            ...
            if sys.platform.startswith("win"):
                try:
                    ...
                    logger.info(
                        f"   ⚠️ Port {port} not found in netstat output (browser may have closed)"
                    )
                except Exception as e:
...
async def cleanup_browser_resources(
    ...
    if browser_pid:
        try:
            ...
            _worker_path = _os.path.join(
                _os.path.dirname(_os.path.abspath(__file__)), "cleanup_worker.py"
            )
            ...
            try:
                ...
                if proc.returncode == 0:
                    logger.info("   ✅ Cleanup worker finished successfully (rc=0)")
                else:
    ...
    if before_count >= 0 and after_count >= 0:
        ...
        if still_alive:
            logger.warning(
                f"   ⚠️ {len(still_alive)} pre-existing Chrome PID(s) still alive: {sorted(still_alive)}"
            )
        if new_orphans:
            logger.error(
                f"   ❌ {len(new_orphans)} new orphaned Chrome PID(s) appeared: {sorted(new_orphans)}"
            )
