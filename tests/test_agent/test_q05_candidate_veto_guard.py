...
class FakeLocator:
    ...
    def __init__(self, count: int, tag: str = 'INPUT', surface: dict = None,
                 box: dict = None):
        self._count = count
        self._tag = tag
    ...
    def nth(self, i: int) -> 'FakeLocator':
        return FakeLocator(1, tag=self._tag, surface=self._surface,
                           box=self._box)
...
class TestQ05Replay:
    ...
    async def test_surface_bearing_mismatch_still_vetoed(self, caplog):
        ...
        with caplog.at_level(logging.INFO):
            result = await _call(
                page, candidate_locator="input[name='username']"
            )
        if result.get('found'):
            assert result['all_locators'][0]['type'] != 'candidate'
    ...
    async def test_empty_surface_non_interactive_not_carved_out(self, caplog):
        ...
        with caplog.at_level(logging.INFO):
            result = await _call(
                page, candidate_locator="div.decor",
