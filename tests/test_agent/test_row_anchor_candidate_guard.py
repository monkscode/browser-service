...
ANCHOR = '4727985745'
DESC = ('Edit icon button inside the table row containing '
        'customer ID 4727985745')
TEXT_EDIT = 'text="Edit"'
...
class TestCandidateAnchorReject:
    ...
    @pytest.mark.asyncio
    async def test_anchor_datum_candidate_rejected_with_expected_text(
        self, caplog
    ):
        with caplog.at_level(logging.INFO):
            result = await _call(
                _q02_page(), candidate_locator='text=4727985745'
            )
    ...
    @pytest.mark.asyncio
    async def test_anchor_datum_candidate_rejected_without_expected_text(
        self, caplog
    ):
    ...
    async def test_row_scoped_candidate_not_rejected(self):
        ...
        result = await _call(
            page, candidate_locator=candidate, expected_text='Edit'
        )
        assert result['found'] is True
        assert result['all_locators'][0]['type'] == 'candidate'
    ...
    async def test_healthy_candidate_with_anchor_set_accepted(self):
        ...
        page = FakePage({
            'id=username': FakeLocator(
                1, text='Enter Account Number or Email', box=EDIT_BOX
            ),
        })
        result = await _call(
            page,
