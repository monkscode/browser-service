...
class TestResolveHiddenInputProxy:
    ...
    async def test_label_for_preferred(self):
        ctx = FakeContext(
            {
                "id=agree": FakeLocator(
                    1,
                    FakeFirst(
                        visible=False,
                        probe=make_probe(
                            id="agree", hasLabelFor=True, hasWrappingLabel=True, siblingTag="span"
                        ),
                    ),
                ),
        ...
        ctx = FakeContext(
            {
                "id=switch23": FakeLocator(
                    ...
                    FakeFirst(
                        visible=False,
                        probe=make_probe(siblingTag="span"),
                    ),
                ),
    ...
    async def test_hidden_no_proxy_flags_only(self):
        ctx = FakeContext(
            {
                "id=orphan": FakeLocator(
                    1,
                    FakeFirst(
                        visible=False,
                        probe=make_probe(id="orphan"),
                    ),
                ),
            }
        )
    ...
    async def test_non_input_tag_no_redirect(self):
        ...
        ctx = FakeContext(
            {
                '[role="switch"][aria-label="Dark mode"]': FakeLocator(
                    1,
                    FakeFirst(
                        visible=False,
                        probe=make_probe(tag="div", id=""),
                    ),
                ),
            }
        )
        result = await resolve_hidden_input_proxy(ctx, '[role="switch"][aria-label="Dark mode"]')
...
class TestTextFirstDetourRedirect:
    async def test_detour_redirects_hidden_input(self):
        ...
        ctx = FakeContext(
            {
                'label:text-is("agree")': FakeLocator(1, FakeFirst(attrs={"for": "agree_box"})),
                'input[id="agree_box"]': FakeLocator(1, FakeFirst(attrs={"type": "checkbox"})),
                "id=agree_box": FakeLocator(
                    1,
                    FakeFirst(
                        visible=False,
        ...
        result = await _find_element_by_expected_text(
            ctx,
            "agree",
            "the checkbox INPUT element for agree",
            x=None,
            y=None,
            iframe_context='iframe[id="main"]',
        )
