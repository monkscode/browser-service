...
async def test_bug2_customer_link_not_navitem(page):
    ...
    await page.goto(_file_url("astpp_create_menu_navitem.html"), wait_until="domcontentloaded")
    await page.evaluate(
        "() => document.querySelectorAll('.dropdown-menu')"
        ".forEach(m => { m.style.display = 'block'; })"
    )
    x, y, data = await _coords_and_data(page, "a.nav-link[href='/customer/create']")
    ...
    assert any(
        token in locator for token in ("customer", "/customer/create", "nav-link", "role=link")
...
async def test_bug3_role_field_strategy_1a_id_anchored(page):
    ...
    await page.goto(_file_url("astpp_customer_form_tom_select.html"), wait_until="domcontentloaded")
...
async def test_multi_instance_tom_select_handler_scoping(page, select_id, expected_value):
    ...
    await page.goto(_file_url("multi_instance_tom_select.html"), wait_until="domcontentloaded")
...
async def test_tom_select_no_label_for_handler_succeeds(page):
    ...
    await page.goto(_file_url("tom_select_no_label_for.html"), wait_until="domcontentloaded")
...
async def test_classifier_picks_up_ts_wrapper_classname_from_real_dom(page):
    ...
    await page.goto(_file_url("astpp_customer_form_tom_select.html"), wait_until="domcontentloaded")
    data = await page.locator("#permission_id + .ts-wrapper").evaluate(_ELEMENT_DATA_JS)
...
async def test_navitem_siblings_get_distinct_locators(page, label):
    await page.goto(_file_url("nav_item_negative.html"), wait_until="domcontentloaded")
...
_CHECKBOX_HIGH = ElementTypeInfo(
    ...
    framework="native",
    confidence="high",
    signals=["tagName=input", "type=checkbox", "tier:0"],
)
...
_CUSTOM_CHECKBOX_HIGH = ElementTypeInfo(
    ...
    framework="custom",
    confidence="high",
    signals=["role=checkbox", "tier:0"],
)
_CUSTOM_RADIO_HIGH = ElementTypeInfo(
    primary_type="radio",
    framework="custom",
    confidence="high",
    signals=["role=radio", "tier:0"],
)
_TOGGLE_HIGH = ElementTypeInfo(
    primary_type="checkbox",
    framework="toggle",
    confidence="high",
    signals=["role=switch", "tier:0"],
)
...
async def test_native_checkbox_id_anchored(page):
    ...
    await page.goto(_file_url("checkbox_radio_widgets.html"), wait_until="domcontentloaded")
    elem_data = {
        "tagName": "input",
        "type": "checkbox",
        "id": "newsletter",
...
async def test_toggle_switch_role_aria_label(page):
    ...
    assert result is not None
    assert result["best_locator"] == '[role="switch"][aria-label="Email notifications"]'
...
async def test_css_in_js_obfuscated_dropdown_classifies_as_combobox(page):
    ...
    await page.goto(_file_url("css_in_js_obfuscated_dropdown.html"), wait_until="domcontentloaded")
...
async def test_shadow_dom_dropdown_confirmed_via_shadow_root_walk(page):
    ...
    await page.goto(_file_url("shadow_dom_dropdown.html"), wait_until="domcontentloaded")
